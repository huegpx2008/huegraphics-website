...
const navigatorGroups: NavigatorGroup[] = [
  {
    ...
    summary:
      "Popular short sleeve, long sleeve, soft-style, and performance tees for DTF transfers.",
    tiers: [
      {
        id: "tee-basics",
        label: "Basics",
    ...
    locations: [
      {
        ...
        width: printWidth,
        height: printHeight,
        widthInches: printWidth,
        heightInches: printHeight,
        colors: "full-color",
      },
    ],
    options: {
...
export function DtfEstimator({ products }: DtfEstimatorProps) {
  ...
  const normalizedQuantity = Math.max(
    dtfMinimumQuantity,
    Math.floor(Number(quantity) || dtfMinimumQuantity),
  );
          ...
          const response = await fetch("/api/pricing/dtf", {
            ...
            body: JSON.stringify(
              buildDtfPayload({
                ...
                printHeight: Number(printHeight),
                bringYourOwnApparel,
              }),
            ),
          });
  ...
  function handleQuantityChange(value: string) {
    ...
    if (value && Number.isFinite(numeric) && numeric < dtfMinimumQuantity) {
      setQuantity(String(dtfMinimumQuantity));
      setMinimumMessage(
        "DTF pricing can start at one piece. Enter at least one item to estimate.",
      );
      return;
    }
  ...
  async function requestDetailEstimate(event: FormEvent<HTMLFormElement>) {
    ...
    if (totalQty < dtfMinimumQuantity) {
      updateDetail({
      ...
      const response = await fetch("/api/pricing/dtf", {
        ...
        body: JSON.stringify(
          buildDtfPayload({
            ...
            printWidth: Number(detailEstimator.printWidth),
            printHeight: Number(detailEstimator.printHeight),
            bringYourOwnApparel: detailEstimator.bringYourOwnApparel,
          }),
        ),
      });
      const data = (await response.json()) as DtfEstimate;
  ...
  function addDetailToBasket() {
    ...
    const item: QuoteBasketItem = {
      ...
      decorationSummary: [
        ...
        `${detailEstimator.printWidth} x ${detailEstimator.printHeight} in.`,
        detailEstimator.bringYourOwnApparel ? "Customer supplied apparel" : "",
      ]
        .filter(Boolean)
        .join(" / "),
      ...
      <div className="mx-auto max-w-[96rem] overflow-hidden rounded-sm bg-[#f4f8fc] shadow-[0_24px_70px_rgba(7,17,31,0.16)] ring-1 ring-black/10">
        <div className="relative bg-[#07111f] p-5 text-white sm:p-7 lg:p-8">
          ...
          <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr] xl:items-end">
            <div>
              <p className="eyebrow text-accent">DTF navigator</p>
            ...
            <div className="grid gap-3 rounded-sm border border-white/10 bg-white/[0.04] p-4 md:grid-cols-[1fr_1fr_1fr_1fr_auto]">
              ...
              <div className="flex flex-col gap-2 md:pt-6">
                <label className="inline-flex min-h-11 cursor-pointer items-center justify-between gap-3 rounded-md border border-white/22 px-4 text-xs font-black uppercase tracking-wide text-white transition hover:border-accent hover:bg-accent/10">
                  <span>BYO apparel</span>
                  <input
                    type="checkbox"
                    checked={bringYourOwnApparel}
                    onChange={(event) =>
                      setBringYourOwnApparel(event.target.checked)
                    }
            ...
            <div className="grid gap-px bg-[#d7e3ee] lg:grid-cols-[1fr_0.8fr]">
              <form onSubmit={requestDetailEstimate} className="bg-white p-5">
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  ...
                  <label className="block">
                    ...
                    <input
                      ...
                      min={1}
                      step="0.25"
                      value={detailEstimator.printWidth}
                      onChange={(event) =>
                        updateDetail({
                          printWidth: event.target.value,
                          estimate: null,
                          error: "",
                        })
                      }
                  ...
                  <label className="block">
                    ...
                    <input
                      ...
                      min={1}
                      step="0.25"
                      value={detailEstimator.printHeight}
                      onChange={(event) =>
                        updateDetail({
                          printHeight: event.target.value,
                          estimate: null,
                          error: "",
                        })
                      }
                  ...
                  <label className="flex min-h-11 cursor-pointer items-center justify-between rounded-md border border-black/12 bg-[#f7f8fa] px-3 text-xs font-black uppercase tracking-wide text-[#314154] sm:mt-6">
                ...
                <div className="mt-5 rounded-md bg-[#eef6ff] p-4 text-sm font-bold leading-6 text-[#125b99]">
                  Item quantity: {getTotalQuantity(detailEstimator.sizes)}. DTF
                  supports short runs, detailed full-color art, and customer
                  supplied garments after review.
                </div>
              ...
              <div className="bg-[#eef4fa] p-5">
                ...
                {detailEstimator.estimate ? (
                  ...
                  <p className="mt-5 rounded-md border border-dashed border-[#b5c6d6] bg-white/70 p-5 text-sm leading-7 text-[#52677d]">
                    ...
                    basket if you are still building the project.
                  </p>
                )}
              </div>
            </div>
