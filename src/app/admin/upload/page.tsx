...
export default async function AdminUploadPage() {
  if (!(await isAdminAuthenticated())) redirect("/admin?next=/admin/upload");
      ...
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-col gap-5 border-b border-white/10 pb-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black sm:text-4xl">Upload Photos</h1>
            <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-[#91a6ba]">Add organized project photos to Cloudinary.</p>
          </div>
          <AdminLogoutButton />
        </header>
        <div className="mt-7">
          <AdminPhotoUploadForm isConfigured={isCloudinaryUploadConfigured()} />
        </div>
      </div>
    </main>
  );
